    ...
    <div className="mb-4">
      ...
      <div className="space-y-2">
        {classes.map((lc) => (
          <div
            key={lc.id}
            className="glass rounded-xl p-3 border border-violet-500/20 flex items-center gap-3"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-violet-400">
                {new Date(lc.scheduled_at).toLocaleDateString("en-IN", {
                  day: "numeric",
                  month: "short",
                  hour: "2-digit",
                  minute: "2-digit",
                })}{" "}
                · {lc.duration_mins} mins
              </div>
            </div>
            <a
              href={lc.meet_link}
...
export default function StudentDashboard() {
        ...
        const withRatings = await Promise.all(
          coursesData.map(async (c) => {
            try {
              ...
              return {
                ...c,
                avg_rating: rat.data.average,
                total_ratings: rat.data.total,
              };
            } catch {
              return { ...c, avg_rating: 0, total_ratings: 0 };
            }
          }),
        );
  ...
  useEffect(() => {
    ...
    courses.forEach(async (c) => {
      try {
        ...
        if (firstYT) {
          const match = firstYT.youtube_url.match(
            /(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&\n?#]+)/,
          );
          if (match) {
  ...
  const toggleWishlist = async (courseId, e) => {
    e.stopPropagation();
    try {
      const r = await api.post("/wishlist/add", { course_id: courseId });
      setWishlisted((w) => ({ ...w, [courseId]: r.data.wishlisted }));
      if (r.data.wishlisted) {
        flash("Added to wishlist ❤️");
      } else {
        flash("Removed from wishlist");
        setWishlist((w) => w.filter((x) => x.course_id !== courseId));
      }
    } catch (e) {}
  };

  const openPreview = async (course, e) => {
    e.stopPropagation();
    try {
      const r = await api.get(`/videos/preview/${course.id}`);
      setPreviewVideo(r.data);
      setPreviewCourse(course);
    } catch (e) {
      flash("No preview available", "error");
    }
  };
  ...
  const openCourse = async (enrollment) => {
    try {
      ...
      flash(e.response?.data?.detail || "Failed to load course", "error");
    }
  };
...
<div class="cert">
  ...
  <div style="font-size:13px;color:#6b7280;">Issued on ${new Date(
    cert.issued_at,
  ...
  const sidebarContent = ({ closeSidebar, collapsed, toggleCollapse }) => (
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">
        {navItems.map(([key, icon, label]) => (
          <button
            ...
            className={`nav-item w-full flex items-center px-3 py-2.5 text-sm transition-all ${
              tab === key ? "active" : ""
      ...
      >
        ...
        <button
          onClick={logout}
          className={`w-full flex items-center px-2 py-2 rounded-xl text-xs text-red-400 hover:bg-red-500/10 transition ${
  ...
  if (playingCourse)
    return (
      ...
      >
          ...
          <div className="flex gap-2 flex-wrap w-full sm:w-auto">
            {[
              ...
              <button
                ...
                className={`text-xs px-3 py-2 rounded-xl transition flex-1 sm:flex-none ${
                  courseTab === t
        ...
        {msg.text && (
          <div
            className={`mx-4 mt-3 text-sm px-4 py-2 rounded-xl border ${
              msg.type === "error"
                ...
                <div className="flex flex-col gap-2 max-h-64 sm:max-h-full overflow-y-auto">
                  ...
                  {videos.map((v, i) => (
                    ...
                    >
                      ...
                      <div
                        className="glass rounded-2xl p-3 border border-white/5 flex-1 min-h-0"
                        ...
                        {submitForm.assignment_id === a.id ? (
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              <button
                                type="submit"
                                className="flex-1 btn-primary text-white py-2 rounded-xl text-sm"
                              >
                                Submit
                              </button>
                              <button
                                ...
                                className="px-4 py-2 rounded-xl text-sm text-gray-400 glass"
                              >
                                Cancel
                              </button>
                            </div>
            ...
            <div className="flex-1">
              {quizResult && (
                <div className="max-w-sm mx-auto text-center animate-fade-up py-8">
                  <div
                    className={`w-28 h-28 rounded-full flex items-center justify-center text-4xl font-bold mx-auto mb-4 ${
                      quizResult.passed
                ...
                <div className="max-w-2xl animate-fade-in">
                            ...
                            <button
                              ...
                              className={`text-left px-4 py-3 rounded-xl text-sm transition border ${
                                answers[q.id] === opt
                                  ...
                                  : "border-white/10 text-gray-300 glass"
                              }`}
                              style={
                                answers[q.id] === opt
                                  ? { background: "rgba(124,58,237,0.3)" }
                  ...
                  <div className="flex gap-3 mt-4">
                    ...
                    <button
                      ...
                      className="glass text-gray-400 px-4 py-2.5 rounded-xl text-sm hover:text-white transition"
                    >
                      Cancel
                    </button>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {q.google_form_url ? (
                            ...
                            <button
                              onClick={() => startQuiz(q)}
                              className="btn-primary text-white px-4 py-2 rounded-xl text-xs font-medium shrink-0"
                            >
                              Start
                            </button>
                          )}
              ...
              >
                <input
                  ...
                  placeholder={
                    replyTo
                      ? "Write a reply..."
                      : "Ask a question or share thoughts..."
                  }
                />
                {replyTo && (
                  <button
                ...
                <button
                  type="submit"
                  className="btn-primary text-white px-4 rounded-xl text-sm"
                >
                  Post
                </button>
              </form>
              ...
              {discussions.map((d) => (
                ...
                >
                  ...
                  {d.replies?.map((r) => (
                    <div
                      key={r.id}
                      className="ml-10 mt-2 p-3 rounded-xl"
            ...
            <div className="flex-1 overflow-y-auto max-w-lg">
              <div className="glass rounded-2xl p-5 border border-white/5 mb-4">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex gap-2 mb-2">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <button
                        key={s}
                        onClick={() =>
                          setMyRating((r) => ({ ...r, rating: s }))
                        }
                        style={{
                          fontSize: 28,
                          color: s <= myRating.rating ? "#fbbf24" : "#374151",
                          background: "none",
                          border: "none",
                          cursor: "pointer",
                        }}
                  ...
                  <textarea
                    value={myRating.review}
                    onChange={(e) =>
                      setMyRating((r) => ({ ...r, review: e.target.value }))
                    }
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const r = await api.get(
                          `/ratings/course/${playingCourse.course_id}`,
                        );
                        setRatings(r.data);
                      ...
                      <div className="flex ml-auto">
                        {[1, 2, 3, 4, 5].map((s) => (
                          <span
                            ...
                            style={{
                              fontSize: 12,
                              color: s <= r.rating ? "#fbbf24" : "#374151",
                            }}
                          >
                            ★
                          </span>
            ...
            <div className="flex-1 max-w-lg">
              {teacherContact ? (
                <div className="space-y-3">
                  ...
                  <div
                    className="p-3 rounded-xl text-xs text-gray-400 text-center"
                    style={{
                ...
                <div className="flex items-center justify-center h-48">
                  <div className="text-center text-gray-600">
                    ...
                    <div className="text-sm">
                      Teacher hasn't added contact details yet
                    </div>
                  </div>
                </div>
              )}
        ...
        <div className="p-4">
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          background:
                            attendance.rate >= 75 ? "#10b981" : "#f59e0b",
                        }}
                      />
                    </div>
                    <div
                  ...
                  {previewVideo.youtube_url ? (
                    <iframe
                      ...
                      src={`https://www.youtube.com/embed/${
                        previewVideo.youtube_url.match(
                          /(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&\n?#]+)/,
                        )?.[1]
                      }?autoplay=1`}
                    ...
                    <video
                      ...
                      src={
                        previewVideo.file_path?.startsWith("http")
                          ...
                          : `https://learnly-lms-hqch.onrender.com${previewVideo.file_path}`
                      }
                    />
                  )}
                </div>
                <div className="mt-3 flex gap-3">
                  {!enrollments.find((e) => e.course_id === previewCourse.id) &&
                    (previewCourse.is_paid ? (
                      <button
                        onClick={() => {
                          setPayingCourse(previewCourse);
                          setPreviewVideo(null);
                          setPreviewCourse(null);
                        }}
                        className="btn-primary flex-1 text-white py-3 rounded-xl font-semibold text-sm"
                      >
                        Buy Now · ₹{previewCourse.price}
                      </button>
                    ) : (
                      <button
                        onClick={() => {
                          enroll(previewCourse.id);
                          setPreviewVideo(null);
                          setPreviewCourse(null);
                        }}
                        className="btn-primary flex-1 text-white py-3 rounded-xl font-semibold text-sm"
          ...
          {tab === "ai-tutor" && (
            <div className="animate-fade-up flex justify-center">
              ...
              >
                <div
                  className="card-base p-4 h-full flex flex-col"
                  style={{ background: "rgba(13,13,26,0.8)" }}
                >
                  <div
                    className="flex items-center gap-3 mb-3 pb-3 shrink-0"
