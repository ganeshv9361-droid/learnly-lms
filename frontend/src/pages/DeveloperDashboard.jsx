...
export default function DeveloperDashboard() {
  ...
  const updateFee = async () => {
    try {
      await api.patch("/developer/platform-fee", {
        platform_fee: parseFloat(newFee),
      });
      loadAll();
      flash(`Fee updated to ₹${newFee}`);
    } catch {
      flash("Error", "error");
    }
  };
  ...
  if (viewAs === "student")
    return (
      <div>
        <div
          className="fixed top-0 left-0 right-0 z-50 flex items-center gap-4 px-4 py-2.5"
          style={{
  ...
  const sidebarContent = ({ closeSidebar, collapsed }) => (
    <div className="flex flex-col h-full">
      ...
      >
        {!collapsed && (
          <div className="flex items-center gap-2.5 p-2.5 rounded-xl mb-2 glass">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs badge-role-dev px-1.5 py-0.5 rounded-md inline-block mt-0.5">
                Dev
              </div>
            </div>
          </div>
  ...
  const msgBar = msg.text ? (
    <div
      className={`text-xs px-3 py-1.5 rounded-xl border animate-fade-in hidden sm:block ${
        msg.type === "error"
  ...
  return (
    <>
      {confirmDelete && (
        ...
        >
          <div
            className="card-base animate-scale-in p-8 max-w-sm w-full text-center"
          ...
          >
            ...
            <div className="text-sm mb-6" style={{ color: "var(--text2)" }}>
              This permanently deletes the user and all their data. This cannot
              be undone.
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmDelete(null)}
                className="flex-1 btn-ghost text-white py-2.5 rounded-2xl text-sm"
              >
                Cancel
              </button>
              <button
      ...
      {msg.text && (
        <div
          className={`fixed bottom-20 left-4 right-4 z-40 sm:hidden text-sm px-4 py-3 rounded-2xl border animate-fade-up ${
            msg.type === "error"
              ? "bg-red-500/10 border-red-500/20 text-red-400"
              : "bg-violet-500/10 border-violet-500/20 text-violet-300"
          }`}
            ...
            <div className="animate-fade-up space-y-4">
              ...
              <div className="space-y-2">
                {filteredUsers.map((u, i) => (
                  <div
                    key={u.id}
                    className={`card-base p-4 flex items-center gap-3 animate-fade-up delay-${Math.min(
                      i * 50,
                      300,
                    )}`}
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="card-base p-4">
                  ...
                  <div className="text-2xl font-bold text-amber-400">
                    ₹
                    {settlements
                      .filter((s) => !s.teacher_paid)
                      .reduce((t, s) => t + s.teacher_amount, 0)
                      .toFixed(0)}
                  </div>
                </div>
                <div className="card-base p-4">
                ...
                {settlements.map((s, i) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded-xl font-medium shrink-0 ${
                          s.teacher_paid ? "badge-free" : "badge-paid"
                        }`}
                      >
                    ...
                    {(s.teacher_upi || s.teacher_account) && (
                      ...
                      >
                        ...
                        {s.teacher_account && (
                          <div style={{ color: "var(--text2)" }}>
                            A/C: {s.teacher_account} · {s.teacher_ifsc}
                          </div>
                        )}
                      </div>
                    )}
                    {!s.teacher_paid && (
                      <button
                        onClick={() => markSettled(s.payment_id)}
                ...
                {[
                  {
                    ...
                    value: earnings.total_platform_earnings,
                    color: "#34d399",
                    prefix: "₹",
                  },
                  {
            ...
            <div className="animate-fade-up space-y-4 max-w-lg">
              <div className="card-base p-5">
                ...
                >
                  ...
                  <div style={{ color: "var(--text2)" }}>
                    Student pays ₹500 → You get ₹{newFee || platformFee} →
                    Teacher gets ₹{500 - (newFee || platformFee)}
                  </div>
                </div>
              </div>
