...
export default function Dashboard() {
  ...
  const avgScore = assessments.length
    ? Math.round(
        assessments.reduce((s, a) => s + a.percentage, 0) / assessments.length,
      )
    : 0;

  const ic =
    "w-full bg-[#252c42] border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-violet-500";
  const lc = "text-xs text-gray-400 mb-1 block";
  const bc =
    "bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition";
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] flex">
      <div className="w-56 bg-[#181c27] border-r border-white/10 flex flex-col p-4 gap-1 shrink-0">
        <div className="text-lg font-semibold text-white px-2 py-3 mb-2">
          ✦ Learnly
        </div>
        ...
        <div className="mt-auto border-t border-white/10 pt-3">
          <div className="text-xs text-gray-500 px-2 mb-0.5">
            {profile.name}
          </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          {[
            ...
            <div
              key={label}
              className="bg-[#181c27] border border-white/10 rounded-xl p-4"
            >
            ...
            <div className="grid grid-cols-2 gap-4">
              {courses.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="bg-[#181c27] border border-white/10 rounded-xl p-5"
                  >
                      ...
                      {enrolled ? (
                        <>
                          <span className="text-xs text-green-400">
                            ✓ Enrolled · {enrolled.progress}% done
                          </span>
                          <button
                        ...
                        <button onClick={() => enroll(c.id)} className={bc}>
                          Enroll
                        </button>
                      )}
                      {enrolled && !hasCert && (
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-400">
                        {e.instructor} · {e.total_modules} modules
                      </div>
                    </div>
                    <button
                      onClick={() => setPlayingCourse(e)}
                      className="text-xs bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 px-3 py-1.5 rounded-lg transition"
                    >
                      ▶ Watch
                    </button>
                  ...
                  {e.total_modules > 0 && (
                    <div className="mb-4">
                      <div className="text-xs text-gray-500 mb-2">
                        Modules:{" "}
                        {Math.round((e.progress / 100) * e.total_modules)} /{" "}
                        {e.total_modules} completed
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        onMouseUp={async (ev) => {
                          ...
                          try {
                            await api.patch(
                              `/enrollments/${e.enrollment_id}/progress`,
                              { progress: val },
                            );
                            loadAll();
                            flash(`Progress updated to ${val}%`);
                          } catch {
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-[#181c27] border border-white/10 rounded-xl p-5">
              ...
              <form onSubmit={submitAssessment} className="space-y-3">
                ...
                <button type="submit" className={bc + " w-full"}>
                  Submit
                </button>
              </form>
            </div>
            <div>
              ...
              {assessments.length === 0 && (
                <div className="text-gray-500 text-sm">No assessments yet.</div>
              )}
              <div className="flex flex-col gap-3">
                {assessments.map((a) => (
                  <div
                    key={a.id}
                    className="bg-[#181c27] border border-white/10 rounded-xl p-4 flex items-center gap-4"
                  >
                    <div className="flex-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {certificates.map((c) => (
                <div
                  key={c.id}
                  className="bg-[#181c27] border border-amber-500/20 rounded-xl p-6"
                >
        ...
        {tab === "profile" && (
          <div className="max-w-lg">
            ...
            <div className="bg-[#181c27] border border-white/10 rounded-xl p-5 flex items-center gap-4 mb-4">
              ...
              <div className="ml-auto text-xs bg-violet-600/20 text-violet-400 px-2 py-1 rounded">
                {profile.role}
              </div>
            </div>
            <div className="bg-[#181c27] border border-white/10 rounded-xl p-5 mb-4">
              ...
              <form onSubmit={saveProfile} className="space-y-3">
                ...
                <button type="submit" className={bc}>
                  Save changes
                </button>
              </form>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {allUsers.map((u) => (
                    <div
                      key={u.id}
                      className="bg-[#181c27] border border-white/10 rounded-xl overflow-hidden"
                    >
                      <div
            ...
            {adminTab === "add-course" && (
              <div className="max-w-md">
                <form
                  ...
                  className="bg-[#181c27] border border-white/10 rounded-xl p-5 space-y-3 mb-5"
                >
                  ...
                  <div>
                    <label className={lc}>Instructor</label>
                    <input
                  ...
                  <div>
                    <label className={lc}>Total modules</label>
                    <input
                      type="number"
                ...
                <div className="flex flex-col gap-2">
                  {courses.map((c) => (
                    <div
                      key={c.id}
                      className="bg-[#181c27] border border-white/10 rounded-lg p-3"
                    >
